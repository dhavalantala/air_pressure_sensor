...
class ModelEvaluation:
    ...
    def initiate_model_evaluation(self)->artifact_entity.ModelEvaluationArtifact:
        try:
            ...
            logging.info(f"Start Finding location of transformer model and target encoder")
            transformer_path = self.model_resolver.get_latest_transformer_path()
            model_path = self.model_resolver.get_latest_model_path()
            target_encoder_path = self.model_resolver.get_latest_target_encoder_path()

            logging.info(f"Load Previous model Objects")
            transformer = load_object(file_path = transformer_path)
            model = load_object(file_path = model_path)
            target_encoder = load_object(file_path = target_encoder_path)

            logging.info(f"Load Current model Objects")
            ...
            logging.info(f"Accuracy using previous train model: {previous_model_score}")
            ...
            logging.info(f"Accuracy using current train model: {current_model_score}")
